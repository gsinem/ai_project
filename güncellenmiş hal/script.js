...
function addOrUpdateEdge() {
    ...
    graph[toNode][fromNode] = weight; // Make graph bidirectional
    
    // Initialize edgeLines if it doesn't exist
    if (!window.edgeLines) {
        window.edgeLines = {};
    }
    
    // Remove existing edge line if it exists
    if (window.edgeLines[`${fromNode}-${toNode}`]) {
        map.removeLayer(window.edgeLines[`${fromNode}-${toNode}`]);
    }
    if (window.edgeLines[`${toNode}-${fromNode}`]) {
        map.removeLayer(window.edgeLines[`${toNode}-${fromNode}`]);
    }
    
    // Add new edge line
    const fromCoords = coordinates[fromNode];
    const toCoords = coordinates[toNode];
    const edgeLine = L.polyline([fromCoords, toCoords], {
        color: '#666',
        weight: 2,
        opacity: 0.8
    }).addTo(map);
    ...
    // Store edge line
    window.edgeLines[`${fromNode}-${toNode}`] = edgeLine;
    window.edgeLines[`${toNode}-${fromNode}`] = edgeLine;
    ...
    // Show success message
    alert(`Edge between ${fromNode} and ${toNode} has been added/updated with weight ${weight} km.`);
}

// Remove a node or edge from the graph
function removeElement() {
    const removeSelect = document.getElementById('remove-element');
    ...
    if (value.startsWith('node:')) {
        // Remove node
        const nodeId = value.substring(5);
        
        // Remove node from graph
        delete graph[nodeId];
        delete coordinates[nodeId];
        
        // Remove edges to this node
        for (const fromNode in graph) {
            delete graph[fromNode][nodeId];
        }
        
        // Remove marker from map
        if (window.nodeMarkers && window.nodeMarkers[nodeId]) {
            map.removeLayer(window.nodeMarkers[nodeId]);
            delete window.nodeMarkers[nodeId];
        }
        
        // If this node was start or end point, clear them
        if (startNode === nodeId) {
            if (startMarker) {
        ...
        // Show success message
        alert(`Node ${nodeId} has been removed successfully.`);
        
    } else if (value.startsWith('edge:')) {
        // Remove edge
        const [, fromNode, toNode] = value.split(':');
        
        // Remove edge from both nodes
        delete graph[fromNode][toNode];
        delete graph[toNode][fromNode];
        
        // Remove edge line from map if it exists
        if (window.edgeLines && window.edgeLines[`${fromNode}-${toNode}`]) {
            map.removeLayer(window.edgeLines[`${fromNode}-${toNode}`]);
            delete window.edgeLines[`${fromNode}-${toNode}`];
        }
        if (window.edgeLines && window.edgeLines[`${toNode}-${fromNode}`]) {
            map.removeLayer(window.edgeLines[`${toNode}-${fromNode}`]);
            delete window.edgeLines[`${toNode}-${fromNode}`];
        }
        ...
        // Show success message
        alert(`Edge between ${fromNode} and ${toNode} has been removed successfully.`);
    }
}
